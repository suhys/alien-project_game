...
class LandingPage:
    ...
    def __init__(self, game):
        ...
        strings = [('SPACE', WHITE, headingFont), ('INVADERS', GREEN, subheadingFont),
               ...
               # ('PLAY GAME', GREEN, font), 
                (f'Highest Score = {str(self.highscore)}', GREY, font)]
        ...
        self.posns.append(700)

        centerx = self.screen.get_rect().centerx

        self.play_button = Button(self.screen, "PLAY GAME", ul=(centerx - 110, 650))
        self.high_score_button = Button(self.screen, "HIGH SCORES", ul=(centerx - 110, 730))
    ...
    def mouse_on_score_button(self):
        mouse_x, mouse_y = pg.mouse.get_pos()
            ...
            if e.type == pg.KEYUP and e.key == pg.K_p:   # pretend PLAY BUTTON pressed
                self.landing_page_finished = True        
                    ...
                    if self.mouse_on_button():
                        self.landing_page_finished = True
                    elif self.mouse_on_score_button():
                        self.hsp.show()
                ...
                if self.mouse_on_button() and not self.hover:
                    ...
                    self.play_button.toggle_colors()
                    self.hover = False
                elif self.mouse_on_score_button() and not self.score_hover:
                    self.high_score_button.toggle_colors()
                    self.score_hover = True
                elif not self.mouse_on_score_button() and self.score_hover:
                    self.high_score_button.toggle_colors()
                    self.score_hover = False
...
class HighScorePage:
    ...
    def __init__(self, game):
        ...
        self.screen = game.screen
        self.highscore_page_finished = False
        self.highscore = game.stats.get_highscore()
        ...
        strings = [('HIGH SCORES', WHITE, headingFont),
                (f'HIGHEST SCORE = {self.highscore:,}', GREY, font)]
        ...
        self.return_button = Button(self.screen, "RETURN", ul=(centerx - 150, 650))
    ...
    def mouse_on_button(self):
        mouse_x, mouse_y = pg.mouse.get_pos()
        return self.return_button.rect.collidepoint(mouse_x, mouse_y)
    ...
    def check_events(self):
        for e in pg.event.get():
            if e.type == pg.QUIT:
                sys.exit()
            if e.type == pg.KEYUP and e.key == pg.K_p:   # pretend PLAY BUTTON pressed
                self.highscore_page_finished = True        
            elif e.type == pg.MOUSEBUTTONDOWN:
                if self.mouse_on_button():
                    self.highscore_page_finished = True
            elif e.type == pg.MOUSEMOTION:
                if self.mouse_on_button() and not self.hover:
                    self.return_button.toggle_colors()
                    self.hover = True
                elif not self.mouse_on_button() and self.hover:
                    self.return_button.toggle_colors()
                    self.hover = False
    ...
    def show(self):
        while not self.highscore_page_finished:
            self.update()
            self.draw()
            self.check_events()   # exits game if QUIT pressed
